...
describe("парсер осевого разбора", () => {
  it("раскладывает оси с фокусом/приоритетом, [ГЛАВНОЕ], [УПРАЖНЕНИЕ]", () => {
    const raw = `Вот разбор.
===IRINAOS===
[ОСЬ 1]
состояние: зона роста
фокус: конкурирующие детали
...
[УПРАЖНЕНИЕ]
Возьмите чужой абзац с тремя деталями и вычеркните две.
Например: из «шляпа, зонт, часы» оставить только «часы» — они тикают.
===КОНЕЦ===`;
    ...
    expect(first.focus).toBe("конкурирующие детали");
    ...
    // у сильной стороны шаг-вопрос сохраняется, приоритет не проставлен
    expect(parsed!.axes[1]!.state).toBe("сильная сторона");
    expect(parsed!.axes[1]!.step).toContain("сжатость");
    expect(parsed!.axes[1]!.priority).toBeUndefined();
    ...
    expect(parsed!.exercise).toContain("Например:"); // демо-пример в упражнении
  });
...
describe("синтез плоского результата для сводки/изысканий", () => {
  it("«разбор» берёт нейтральный фокус (не имя оси), «точка роста»/«упражнение» на месте", () => {
    const parsed = axisResultToParsed({
      axes: [
        {
          key: "g",
          label: "1. Деталь вместо описания",
          state: "зона роста",
          focus: "конкурирующие детали",
          seen: "три детали",
          step: "какая понесёт?",
        },
    ...
    expect(parsed["разбор"]).toContain("конкурирующие детали (зона роста): три детали");
    expect(parsed["разбор"]).toContain("→ какая понесёт?");
    expect(parsed["разбор"]).not.toContain("Деталь вместо описания"); // имя оси не выдаём
